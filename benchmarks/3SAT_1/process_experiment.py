...
#time_y = []
for j, i in enumerate(list(df['bits'].unique())):
    data_time_1_P.append({'bits': df[df['bits'] == i]['bits'], 'time':1/(df[df['bits'] == i]['solution_count']/2**i), '1_P':df[df['bits'] == i]['total_time']})
#    satisfiability_x += list(1/(df[df['bits'] == i]['solution_count']/2**i))
#    time_y += list(df[df['bits'] == i]['total_time'])
    plt.scatter(1/(df[df['bits'] == i]['solution_count']/2**i), df[df['bits'] == i]['total_time'], label='{0} bits'.format(i), facecolors='none', edgecolors=colors[j], s=10)
#y = func(np.unique(satisfiability_x))
...
plt.savefig('results_CNF/satisfiability_time_2')
plt.clf()

df_time_1_P = pd.DataFrame(data_time_1_P)
df_time_1_P.to_csv('results_CNF/time_1_P.csv')
...
plt.legend()
plt.title('Satisfiability vs time to construct on all bits')
plt.savefig('results_CNF/satisfiability_construction_time_2')
plt.clf()
...
plt.legend()
plt.title('Satisfiability vs tree time on all bits')
plt.savefig('results_CNF/satisfiability_tree_time_2')
plt.clf()
...
plt.legend()
plt.title('Satisfiability vs time to contract on all bits')
plt.savefig('results_CNF/satisfiability_contraction_time_2')
plt.clf()
...
plt.savefig('results_CNF/density_time_nodes')
plt.clf()
...
plt.savefig('results_CNF/nodes_mean_time')
plt.clf()
...
plt.savefig('results_CNF/nodes_mean_time_density')
plt.clf()
